...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb):  #它的输入是x和emb
        for layer in self:  #遍历内部的所有层网络。
            if isinstance(layer, TimestepBlock):  #只有当该层是TimestepBlock对象时，才额外传入emb。只有resblock是TimestepBlock子类，所以除了resblock外其他模块都不需要传入emb
                x = layer(x, emb)
            else:
                x = layer(x)
        return x

class Upsample(nn.Module):#使用插值法进行上采样；可以选择是否再使用卷积。
    """
...
class Downsample(nn.Module):   #可以选择卷积或者平均池化来进行下采样操作。
    """
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.use_conv = use_conv
        self.dims = dims
        stride = 2 if dims != 3 else (1, 2, 2)  #当维度为3时；stride为元组(1, 2, 2)
        if use_conv:
            self.op = conv_nd(
...
def conv_dw(inp, oup, stride):
    return nn.Sequential(
        # dw
        nn.Conv2d(inp, inp, 3, stride, 1, groups=inp, bias=False),  #当groups与输入通道数相等时，此卷积就是逐通道卷积。
        nn.BatchNorm2d(inp),
        nn.ReLU(inplace=True),
...
class ResBlock(TimestepBlock):  #包含输入层、嵌入层、输出层。
    ...
    def __init__(
        ...
        super().__init__()
        self.channels = channels    #channels是输入通道数
        ...
        self.in_layers = nn.Sequential(
            normalization(channels),  #channels是输入通道数
            nn.SiLU(),
            conv_nd(dims, channels, self.out_channels, 3, padding=1),
        )

        self.updown = up or down  #如果 up 或 down 任意一个为 True，则 self.updown 被赋值为 True。；如果 up 和 down 同时为 False，则 self.updown 被赋值为 False。

        if up:
            # channels是输入通道数
            self.h_upd = Upsample(channels, False, dims)   #False表示使用平均池化来进行下采样，而不采用卷积进行下采样。；dims表示几维（2、3D）操作；
        ...
        self.emb_layers = nn.Sequential(
            ...
            linear(
                emb_channels,
                2 * self.out_channels if use_scale_shift_norm else self.out_channels,  #该表达式如果条件为真返回2倍的self.out_channels；否则返回self.out_channels。
            ),
        )
        self.out_layers = nn.Sequential(
            ...
            #zero_module()函数返回的模型其参数经过.detach()处理，不能计算梯度。
            zero_module(
                conv_nd(dims, self.out_channels, self.out_channels, 3, padding=1)
            ),
        )
       #如果输入和输出通道数相等，直接使用 nn.Identity() 函数来保持输入不变。
        # 如果输入和输出通道数不相等，且使用卷积进行通道数调整（use_conv=True），则创建一个卷积层来调整通道数，以便进行跳跃连接。
    ...
    def _forward(self, x, emb):
        if self.updown:  #如果up 为 True，则 self.updown 被赋值为 True。此时对输入序列模块进行上采样；如果down为true，则self.updown被赋值为true，此时对输入序列进行下采样。
            #in_conv：输入序列中最后一层，in_rest：除了最后一层的其他部分层。
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1] #[-1] 读取倒数第一个元素；[:-1]--返回除了位置为-1元素的列表；[::-1] 顺序相反操作；[3::-1] 从下标为3（从0开始）的元素开始翻转读取
            h = in_rest(x)
            h = self.h_upd(h)
            x = self.x_upd(x)
            h = in_conv(h)
            ...
            h = self.in_layers(x)
        emb_out = self.emb_layers(emb).type(h.dtype)   #嵌入序列是对时间嵌入数据进行处理的。调用 tensor.dtype 可以返回张量中数据的具体类型。如torch.DoubleTensor，torch.FloatTensor，torch.LongTensor；tensor.type()是将tensor的数据类型转化为函数形参中的数据类型。
        while len(emb_out.shape) < len(h.shape):   #这里比较的两个张量谁的维度更高，如果嵌入序列输出张量的维度更低，那么就对其进行扩维，直到两个张量的维度数相同，循环停止。；tensor.shape()函数返回的是关于张量维度的一个元组。；元组的大小比较遵循“按索引依次比较对应元素”原则；不过这里比较的是元组里面元素；len()方法返回对象（字符、列表、元组等）长度或项目个数。。
            emb_out = emb_out[..., None]  #emb_out[..., None] 是一种高效的方式来在张量 emb_out 的最后一个维度上增加一个维度。
        if self.use_scale_shift_norm:
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]   #[1:]截取索引1到所有。
            scale, shift = th.chunk(emb_out, 2, dim=1) #torch.chunk(tensor, chunk_num, dim)与torch.cat()原理相反，它是将tensor按dim（行或列）分割成chunk_num个tensor块，返回的是一个元组。
...
class UNetModel_v1preview(nn.Module):
    ...
    def __init__(
        ...
        self.attention_resolutions = attention_resolutions  #attention_resolutions表示在哪些位置上做多头注意力机制（并不是每个resblock后面都会做attention）
        self.dropout = dropout
        ...
        time_embed_dim = model_channels * 4     #时间嵌入的嵌入维度
        self.time_embed = nn.Sequential(#对输入到unet里面的timestep进行变换。是一个两维张量
            linear(model_channels, time_embed_dim), #张量变换：(b,model_channels)  -> (b,time_embed_dim)
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        )

        if self.num_classes is not None: #条件生成情况下，对标签进行embedding。
            self.label_emb = nn.Embedding(num_classes, time_embed_dim)      #torch.nn.Embedding(num_embeddings, embedding_dim, padding_idx=None, max_norm=None, norm_type=2.0, scale_grad_by_freq=False, sparse=False, _weight=None, _freeze=False, device=None, dtype=None)
                                                                           #参数： ① num_embeddings (int)： 词典中词的总数② embedding_dim (int)： 词典中每个词的嵌入维度  ；padding_idx可用于指定词典中哪一个索引的词填充为0。

        #nn.ModuleList,它是一个存储不同module，并自动将每个module的parameters添加到网络之中的容器。
        self.input_blocks = nn.ModuleList(    #self.input_blocks它的输入是x，emb。
            [
        ...
        for level, mult in enumerate(channel_mult):  #channel_mult=(1, 2, 4, 8)
            ...
            if level != len(channel_mult) - 1:  #channel_mult是一个含有四个元素的元组(1, 2, 4, 8)；这里的意思是说如果不是最后一次循环执行下面语句。
                out_ch = ch
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if resblock_updown
                            ...
                            ch, conv_resample, dims=dims, out_channels=out_ch    #ch代表通道；conv_resample为bool类型，表示是否使用卷积进行下采样。
                        )
                    )
                )
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:  #channel_mult是一个元组：(1, 2, 4, 8)；a[:-1]除了最后一个取全部；a[::-1]取从后向前（相反）的元素；a[2::-1]取从下标为2的元素翻转读取。
    ...
    def convert_to_fp16(self):
        ...
        """
        self.input_blocks.apply(convert_module_to_f16)  #apply(fn)的官网介绍，该方法会将fn递归的应用于模块的每一个子模块（.children()的结果）及其自身。convert_module_to_f16函数是将卷积层的权重偏差精度改为半精度.
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []  #记录输入模块self.input_blocks每一层的输出张量。
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))   #timestep_embedding：创建一个正弦时间步嵌入张量；self.time_embed函数是一个Sequential容器，为了提取时间步张量的特征。

        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)  #label_emb()进行一个nn.Embedding(num_class, time_emb_dim)函数操作。将标签嵌入张量和时间步嵌入张量逐元素相加进行特征融合。

        h = x.type(self.dtype)  #对张量x进行数据类型转换。
        c = h[:,:-1,...]  #切片操作不要最后一个通道的数据，其余全要。
        hlist= []
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:   #emb张量的维度数大于2时；
                emb = emb.squeeze()
            h = module(h, emb)
            hs.append(h)
        uemb, cal = self.highway_forward(c, [hs[3],hs[6],hs[9],hs[12]])  #第一个返回值是模型最后一层卷积的输出。这个值通常被称为特征向量或嵌入（Embedding），它包含了输入图像经过模型处理后提取到的特征信息。
                                                                         #第二个返回值是模型的预测结果。对于语义分割任务，它表示了模型对输入图像进行分割后得到的预测结果。
        h = h + uemb
...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        out_channels,
        num_res_blocks,
        attention_resolutions,  ##attention_resolutions表示在哪些位置上做多头注意力机制（并不是每个resblock后面都会做attention）
        dropout=0,
        channel_mult=(1, 2, 4, 8),
        conv_resample=True,
        dims=2,
        ...
        self.input_blocks = nn.ModuleList(  #self.input_blocks它的输入是x，emb。，但是只有在resblock中才会输入emb。
            [
                TimestepEmbedSequential(
                    conv_nd(dims, in_channels, model_channels, 3, padding=1)
                )
        ...
        for level, mult in enumerate(channel_mult):   #channel_mult=(1, 2, 4, 8)
            ...
            for _ in range(num_res_blocks):
                ...
                ch = mult * model_channels
                if ds in attention_resolutions:# 如果符合条件就会插入注意力模块，
                    layers.append(
                        AttentionBlock(
                            ch,
                            use_checkpoint=use_checkpoint,
    ...
    def forward(self, x, timesteps, y=None):  #y是条件，用于条件生成模型，这里不需要
        """
        Apply the model to an input batch.
        ...
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)  #label_emb()进行一个nn.Embedding(num_class, time_emb_dim)函数操作。将标签嵌入张量和时间步嵌入张量逐元素相加进行特征融合。
        #上面是对timestep emb和条件emb的处理。接下来开始对模块进行数据输入。
        h = x.type(self.dtype)  #对张量x进行数据类型转换。
        c = h[:,:-1,...]    #切片操作不要最后一个通道的数据，其余全要。
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        if conv_op == nn.Conv2d:
            ...
            transpconv = nn.ConvTranspose2d    #反卷积
        ...
        for d in range(num_pool):
            ...
            # StackedConvLayers是condition encoder？？
    ...
    def forward(self, x, hs = None):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            if hs:
                ...
                hb = th.mean(h,(2,3))
                hb = hb[:,:,None,None]
                x = x * ha * hb     #进行attentive mechanism 操作 ；   X是从源图提取的特征张量；ha和hb是在扩散过程中提取的特征张量。
            

        x = self.conv_blocks_context[-1](x)   #进行condition encoder操作
        emb = conv_nd(2, x.size(1), 512, 1).to(device = x.device)(x)

        for u in range(len(self.tu)):    #self.tu是一个包含反卷积（上采样）操作的列表。
            x = self.tu[u](x)
            x = th.cat((x, skips[-(u + 1)]), dim=1)   #将同层次的两个编码器输出进行拼接操作。
            x = self.conv_blocks_localization[u](x)    #进行分割编码器操作
            if self._deep_supervision:
                seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x)))   #self.final_nonlin()函数其实就是：sigmoid_helper = lambda x: F.sigmoid(x)
