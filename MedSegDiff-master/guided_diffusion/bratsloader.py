...
class BRATSDataset(torch.utils.data.Dataset):
    def __init__(self, directory, transform, test_flag=False):
        ...
        '''
        super().__init__()
        #用于将路径字符串中的波浪线（~）扩展为用户的主目录。它的作用是提供跨平台的路径展开功能，使得路径可以在不同的操作系统上正确地解析。
        ...
        #set顾名思义是集合，里面不能包含重复的元素，接收一个list作为参数,过滤掉重复的元素.
        self.seqtypes_set = set(self.seqtypes)
        self.database = []
        #遍历目录,子目录,文件;如果有子目录下此遍历子目录,子目录的子目录,文件.如此知道没有子目录为止.
